...
def nocs_grid_sample(feature_volume: torch.Tensor, query_points: torch.Tensor,
    ...
    # TODO: looks very important but why?
...
class VirtualGrid:
    ...
    def get_points_grid_idxs(self, points, batch_idx=None):
        """
        Get back to original space [0,32], and add batch index. and clamping
        """
        lower_corner = self.lower_corner
        upper_corner = self.upper_corner
        grid_shape = self.grid_shape
        ...
        points_idxs_f = (points + offsets) * scales  # num_ptsx3, back to [0, 32]
        ...
        if batch_idx is not None:  # num_ptsx4, add batch idx
    ...
    def flatten_idxs(self, idxs, keepdim=False):
        ...
        flat_idxs = (idxs * torch.tensor(target_stride,  # most important step
    ...
    def idxs_to_points(self, idxs):
        """
        Normalize the predicted coordinates  to [0,1]
        """
        lower_corner = self.lower_corner
        upper_corner = self.upper_corner
        grid_shape = self.grid_shape
