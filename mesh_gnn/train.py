...
from utils.misc_utils import vv_to_args, set_resource
import copy
from mesh_gnn.mesh_dyn import MeshDynamics
from mesh_gnn.vc_edge import VCConnection
from chester import logger
from utils.misc_utils import configure_logger, configure_seed
...
def get_default_args():
    ...
    parser.add_argument('--cached_states_path', type=str, default='Trousers_2500_v1.pkl')
    parser.add_argument('--num_variations', type=int, default=2500)
    ...
    # Dataset
    parser.add_argument('--n_rollout', type=int, default=5000, help='Number of training trajectories')
    parser.add_argument('--time_step', type=int, default=80, help='Time steps per trajectory')
    ...
    parser.add_argument('--neighbor_radius', type=float, default=0.036, help="Radius for connecting nearby edges")
    ...
    # Training
    parser.add_argument('--train_mode', type=str, default='full', help='Should be in ["vsbl", "full"]')
...
def create_env(args):
    ...
    env_args['cloth3d_dir'] = args.cloth3d_dir
    ...
    env_args['cloth_type'] = args.cloth3d_type
...
def main():
    ...
    if args.edge_model_path is not None:
        ...
        vcd_edge = None
    vcdynamics = MeshDynamics(args, env, vcd_edge=vcd_edge)  # Input the vcd_edge model in case we want to train with predicted edges
