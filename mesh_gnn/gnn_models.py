...
class NodeModel(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, u, batch):
        ...
        # pickle.dump((x, edge_index, edge_attr, u, batch, edge_attr_aggregated), open("t.pkl", "wb"))
        # pdb.set_trace()
        out = self.model(model_input)
        return out
...
class GNBlock(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, u, batch):
        ...
        # pickle.dump((x, edge_attr, u), open("t.pkl", "wb"))
        # pdb.set_trace()
...
class Processor(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, u, batch):
        ...
        #     pickle.dump((x_new, edge_index, edge_attr_new, u_new, batch, x_res, edge_attr_res, u_res), open("t.pkl", "wb"))
        #     pdb.set_trace()
        #     x_new = x_new + x_res
        #     edge_attr_new = edge_attr_new + edge_attr_res
        #     u_new = u_new + u_res
        # return x_res, edge_attr_res, u_res  # old model train on this but it's not right
        # # return x_new, edge_attr_new, u_new
    ...
    def load_model(self, model_path, load_names='all', load_optim=False, optim=None):
        ...
        if load_names == 'all':
            for k, v in self.dyn_models.items():
                if k not in ckpt:
                    self.dyn_models[k].load_state_dict(ckpt["processor_a"])
                else:
